...
import { joinEvent, cancelJoinEvent, addBookmark, deleteBookmark, readEvent } from '../slices/eventsSlice';
      ...
      <div className="px-5 pt-4 pb-1">
        <div className="mb-3">
          ...
          <div className="flex space-x-4 text-sm mb-2">
            ...
            <div>
              <a className="font-medium text-gray-800" href="#0">
                {`⏳ ${moment(event?.end_time).diff(moment(event?.start_time), 'minutes')} mins`}
              </a>
            </div>
          </div>
          {/* Title */}
          <h2 className="font-semibold text-2xl text-gray-800">{event?.title}</h2>
          <a className="font-light text-red-600" href="#0">
            {locations.entities[event?.location_id]?.name}
          </a>
        </div>
        <div className="mb-5">
          ...
          <div className="mb-3">
            ...
            <AvatarAndName
              real_name={accounts.entities[event?.creator_account_id]?.real_name}
              username={accounts.entities[event?.creator_account_id]?.username}
            />
          </div>
